...
from phone.utils.file_utils import *
from phone.utils.airtest_utils import *
from phone.target.Ids import *
import time,random
...
def autoFollow():
    target = SmallTarget()
    target.run()
    target.new_money=0.0
    global EARNED_MONEY
    if EARNED_MONEY>0:
        target.new_money = target.money-EARNED_MONEY
    EARNED_MONEY=target.money
    return target
